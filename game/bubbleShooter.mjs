...
export class BubbleShooterGame {
  ...
  addEventListeners() {
    try {
      ...
      if (playAgainBtn) playAgainBtn.onclick = () => this.showModeSelection();
      if (backToMenuBtn) backToMenuBtn.onclick = () => this.exitGame();
      ...
      if (this.canvas) {
        this.canvas.addEventListener('mousemove', (e) => {
          try {
            ...
            console.error('❌ Error in aim event:', error);
          }
        });
        ...
        this.canvas.addEventListener('click', (e) => {
          try {
            ...
            console.error('❌ Error in shoot event:', error);
          }
        });
      }
  ...
  spawnShootingBubble() {
    try {
      ...
      if (Math.random() < 0.6 && this.colorDistribution && this.colorDistribution.size > 0) {
      ...
      console.log('✅ Shooting bubble spawned:', type);
    } catch (error) {
      console.error('❌ Error spawning shooting bubble:', error);
      // Fallback creation
      this.shootingBubble = {
        type: 'blue',
        x: this.canvas.width / 2,
